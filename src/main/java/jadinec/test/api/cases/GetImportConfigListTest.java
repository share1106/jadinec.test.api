...
public class GetImportConfigListTest {

	public static String stringId = null;
	// 列表分页
	@Test
	public void getImportConfigListTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("tableName").toString().contains("路面横断面面积数据表")) {
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.getImportConfigListUrl);
		System.out.println(TestConfig.getImportConfigListUrl);
