...
public class GetOrgProListByPageTest {

	public static String stringId = null;
	// 分页获取项目部列表
	@Test
	public void getOrgProListByPageTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("proName").toString().contains("测试新增项目3")) {
				stringId = jsonObj.get("proId").toString();
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.getOrgProListByPageUrl);
		System.out.println(TestConfig.getOrgProListByPageUrl);
		JSONObject param1 = new JSONObject();
		JSONObject param2 = new JSONObject();
		JSONObject param3 = new JSONObject();

		param3.put("content", "新增");
