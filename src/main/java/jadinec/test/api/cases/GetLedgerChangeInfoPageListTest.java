...
public class GetLedgerChangeInfoPageListTest {

	public static String stringId;

	// 变更信息-分页列表
	@Test
	public void getLedgerChangeInfoPageListTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("changeName").toString().contains("测试台账变更名称888")) {
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.getLedgerChangeInfoPageListUrl);
		System.out.println(TestConfig.getLedgerChangeInfoPageListUrl);
		JSONObject param1 = new JSONObject();
		JSONObject param2 = new JSONObject();

		param2.put("proCode", "2");// 项目编号
		param2.put("pageNum", 1);// 当前页 默认1
		param2.put("pageSize", 20);// 页大小 默认10
