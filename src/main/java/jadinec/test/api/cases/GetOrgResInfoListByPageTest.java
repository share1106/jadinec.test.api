...
public class GetOrgResInfoListByPageTest {
	
	public static String stringId = null;

	// 分页获取资源列表
	@Test
	public void getOrgResInfoListByPageTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("resName").toString().contains("测试资源新增")) {
				stringId = jsonObj.get("resId").toString();
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.getOrgResInfoListByPageUrl);
		System.out.println(TestConfig.getOrgResInfoListByPageUrl);
		JSONObject param1 = new JSONObject();
		JSONObject param2 = new JSONObject();
		JSONObject param3 = new JSONObject();
		
		param3.put("resName", "");// 资源名称
