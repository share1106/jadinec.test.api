...
public class GetOrgJobDicListByPageTest {

	public static String stringId = null;
	// 分页获取岗位编码字典列表数据
	@Test
	public void getOrgJobDicListByPageTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("jobCodeName").toString().contains("测试岗位名称")) {
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.getOrgJobDicListByPageUrl);
		System.out.println(TestConfig.getOrgJobDicListByPageUrl);
