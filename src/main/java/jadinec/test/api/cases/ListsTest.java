...
public class ListsTest {
	...
	public void listsTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("taxName").toString().contains("测试税收类别1")) {
				stringId = jsonObj.get("taxId").toString();
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.listsUrl);
		System.out.println(TestConfig.listsUrl);
