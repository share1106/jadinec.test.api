...
public class GetOrgRoleInfoListByPageTest {

	public static String stringId = null;
	// 分页获取组织机构角色列表
	@Test
	public void getOrgRoleInfoListByPageTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("roleName").toString().contains("测试角色")) {
				stringId = jsonObj.get("roleId").toString();
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.getOrgRoleInfoListByPageUrl);
		System.out.println(TestConfig.getOrgRoleInfoListByPageUrl);
		JSONObject param1 = new JSONObject();
		JSONObject param2 = new JSONObject();
		JSONObject param3 = new JSONObject();
		
		param3.put("orgId", "-1");
		param3.put("roleName", "测试角色1");
