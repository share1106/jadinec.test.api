...
public class GetOrgUserListByPageTest {

	public static String stringId = null;
	// 根据用户机构分页查询用户信息
	@Test
	public void getOrgUserListByPageTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("contactNumber").toString().contains("18611599121")) {
				stringId = jsonObj.get("userId").toString();
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.getOrgUserListByPageUrl);
		System.out.println(TestConfig.getOrgUserListByPageUrl);
		JSONObject param1 = new JSONObject();
		JSONObject param2 = new JSONObject();
		JSONObject param3 = new JSONObject();

		param3.put("content", "");
		param3.put("userType", 0);// 用户类型 0正式员工 1外聘员工 2产业工人 3其它
