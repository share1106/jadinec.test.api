...
public class GetMaterialRecordListTest {

	public static String stringId = null;

	// 获取物料0#台账变更管理列表
	@Test
	public void getMaterialRecordListTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("materialName").toString().contains("热轧光圆钢筋（HPB300-φ12）")) {
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.getMaterialRecordListUrl);
		System.out.println(TestConfig.getMaterialRecordListUrl);
		JSONObject param1 = new JSONObject();
		JSONObject param2 = new JSONObject();

		param2.put("changeStartDate", "");
		param2.put("changeEndDate", "");
		param2.put("proCode", "16");
		...
		post.setHeader("accessToken", ConfigFile.access_token_pc_pro);
