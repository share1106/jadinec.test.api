...
public class GetBillCodeProcessCodeListTest {

	public static String stringId = null;

	// 清单工序管理列表
	@Test
	public void getBillCodeProcessCodeListTest() throws IOException {
		...
		for (int k = 0; k < ja.size(); k++) {
			...
			if (jsonObj.get("ledgerCode").toString().contains("401-2") && 
					jsonObj.get("processCode").toString().contains("GX0401")) {
				stringId = jsonObj.get("templateId").toString();
	...
	private JSONArray getJsonResult() throws ClientProtocolException, IOException {
		HttpPost post = new HttpPost(TestConfig.getBillCodeProcessCodeListUrl);
		System.out.println(TestConfig.getBillCodeProcessCodeListUrl);
		JSONObject param1 = new JSONObject();
		JSONObject param2 = new JSONObject();

		param2.put("keyWords", "");// 模糊查询条件
		param2.put("pageNum", 1);// 当前页
		param2.put("pageSize", 20);// 页大小
